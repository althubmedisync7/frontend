...
// --- Data (Updated to match the data in the new mobile image where possible) ---
const data = [
...
// --- Reusable Components (Keep them here) ---
...
const DeleteAccountModal = ({ isVisible, onClose, onDelete }) => (
   // ... (Your AntD Modal component remains the same for both views)
   <Modal
      centered
...
const MobileAccessCard = ({ date, access, doctorName, hospitalName }) => (
   <div className="bg-white p-4 shadow-sm border border-gray-200 rounded-lg mb-3">
      {/* Top Row: Date and Toggle Switch */}
      <div className="flex justify-between items-center mb-2">
         <span className="text-gray-900 font-semibold">{date}</span>
         <ToggleSwitch checked={true} />
      </div>

      {/* Middle Row: Access Dropdown */}
      <div className="flex items-center mb-2">
         {/* The Select component is styled to look like the mobile image dropdown */}
         <Select
            defaultValue={access}
            bordered={false} // AntD styling removal
      ...
      {/* Bottom Rows: Doctor and Hospital Details */}
...
const PrivacyDataSharing = () => {
   ...
   // --- DESKTOP TABLE COLUMNS (Only needed for the desktop view) ---
   const columns = [
   ...
   if (isMobile) {
      return (
         <div className="w-full bg-gray-50 min-h-screen pt-12">
            {/* Header for Mobile Detail Screen (Simulates the parent settings page's header) */}
            <div className="fixed top-0 left-0 right-0 z-10 bg-white border-b border-gray-200">
               <div className="flex items-center p-4">
                  {/* Back Arrow - You'd handle navigation here */}
   ...
   // --- DESKTOP VIEW RENDER (Original component logic) ---
   return (
      <div className="w-full mx-auto bg-white rounded-lg p-6">
         {/* ... (Original Desktop/AntD Table rendering logic) ... */}
