...
import { Pagination, Dropdown, Menu, Space } from 'antd'; // Imported Dropdown, Menu, and Space
    ...
    {results.map(result => (
      ...
      >
        ...
        <div className="flex flex-col space-y-2">
          ...
          <button className="w-full px-3 py-3 text-indigo-700 bg-white border border-gray-300 hover:bg-gray-50 rounded-lg text-sm transition-colors">
            Share
          </button>
        </div>
      </div>
...
// Menu component for the Ant Design Dropdown
const filterMenu = (
  <Menu
    onClick={e => console.log('Filter selected:', e.key)} // Replace with actual filter logic
    items={[
...
const PatientResult = () => {
  ...
  const [pageSize, setPageSize] = useState(6); // 6 items per page for visual demo

  // State for the selected filter item (for display purposes)
  ...
  const TableView = () => (
    // Hide the Table on screens smaller than md (tablet)
    <div className="mt-6 hidden md:block">
      <div className="bg-white rounded-xl shadow overflow-x-auto">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-white border-b border-gray-200">
            <tr>
              {/* Ensured table headers stay compact on tablet */}
              {[
                'Date',
                'Test Name',
                'Hospital/Lab Name',
            ...
            {currentResults.map(result => (
              <tr key={result.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                  <button
                    onClick={() =>
                      navigate(`/patientboard/results/${result.id}`)
                    }
      ...
      <div className="mt-6 flex justify-between items-center text-sm text-gray-600">
        <div className="flex items-center">
          You have <span className="font-bold mx-1">{totalResults.length}</span>{' '}
          laboratory results (Displaying{' '}
          <span className="font-bold mx-1">{currentResults.length}</span> per
          page)
        </div>
  ...
  return (
    <div>
      {/* Header Area (Title, Subtitle, Search, Dropdown) */}
      <div className="mb-6 flex flex-col md:flex-row md:justify-between md:items-start">
        ...
        <div className="mb-4 md:mb-0">
          ...
          <p className="text-gray-500 mt-1">
            Get updated with your test results.
          </p>
        </div>

        {/* Search & Filter Block: Uses flex-wrap for mobile stacking */}
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* --- REPLACED WITH ANTD DROPDOWN START --- */}
          <Dropdown
            overlay={filterMenu}
            trigger={['click']}
            className="w-full sm:w-auto" // Apply full width on mobile, auto on sm+
          >
            {/* Custom trigger button styled with Tailwind */}
          ...
          {/* --- REPLACED WITH ANTD DROPDOWN END --- */}
        </div>
      </div>

      {/* Content: Switch between Table and Card View based on screen size */}
      {hasResults ? (
        <>
      ...
      {/* Centralized Pagination for Mobile/Card View */}
      {hasResults && (
        <div className="mt-6 flex justify-center md:hidden">
          <Pagination
            ...
            showLessItems={true}
            hideOnSinglePage={true}
            simple={true} // Use simple mode for better mobile display
            itemRender={(page, type, originalElement) => {
              if (type === 'prev') return '← Previous';
              if (type === 'next') return 'Next →';
              return originalElement;
